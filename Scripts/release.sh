...
echo ""
echo "다음 단계:"
# 화면이 바뀐 릴리스에서 스크린샷을 갱신하지 않으면, 사이트와 README가 이전 버전의
# 결함을 계속 광고하게 된다. 반대로 미리 갈아끼우면 배포되지 않은 화면을 광고하는
# 셈이므로, 교체 시점은 정확히 여기다 — 게시 직전.
echo "  1) UI가 바뀌었다면 스크린샷부터 갱신:"
echo "       Scripts/capture-localized.sh build/shots"
echo "       cp build/shots/verdict-en.png        docs/images/verdict.png"
echo "       cp build/shots/verdict-system-en.png docs/images/verdict-system.png"
echo "       cp build/shots/inventory-en.png      docs/images/inventory.png"
echo "       cp build/shots/permission-en.png     docs/images/permission.png"
echo "     (docs/index.html과 15개 README가 이 파일들을 그대로 참조한다)"
echo "  2) gh release create v$VERSION $DMG --title v$VERSION --notes-file <notes>"
echo "  3) Scripts/update-cask.sh $VERSION"
