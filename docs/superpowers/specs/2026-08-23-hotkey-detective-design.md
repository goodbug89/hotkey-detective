...
1. `certain` 증거의 owner 집합이 2개 이상 → `contested`. 1개면: 다른 owner가 `high` 이상 증거를 가지면 `contested`(certain 우선 정렬), 아니면 `confirmed`. *(2026-08-23 개정: ⌃Space에서 시스템 certain + Maccy high가 confirmed로 나와 11절 체크리스트의 contested 기대와 어긋남을 실기로 확인, 규칙을 의도에 맞게 수정)*
   - **[실기 검증 2026-08-24]** 세 케이스로 규칙을 가둠: ⌘Space(시스템 certain + 반응 high/observation) → `confirmed`; ⌃Space + Maccy 미실행(시스템 certain + Maccy low/claim) → `confirmed` + "실행 시 충돌 예상" 경고; ⌃Space + Maccy 실행(시스템 certain + Maccy high/claim) → `contested`. 구분이 필요한 만큼만 걸림을 확인.
...
- `REACTION_DELAY` 300ms가 Raycast/Alfred 창 표시에 충분한지 실측.
  - **[검증됨 2026-08-23]** ⌃Space → `contested(이전 입력 소스 선택 (시스템), Maccy · togglePreview)`, 근거 2건 — 개정된 규칙 1(certain vs high 충돌)의 실기 확인. 이로써 confirmed/likely/contested/free 전부 실기 검증됨; occupiedUnknown은 Carbon 신호의 크로스 프로세스 한계로 v1에서 도달 불가(6.2절 참고).
