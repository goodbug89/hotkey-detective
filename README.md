...
> **재빌드마다 권한을 다시 허용해야 합니다.**
> 이 앱은 ad-hoc 서명(개발자 인증서 없이 서명)으로 번들을 만듭니다. macOS는 서명이 바뀐 바이너리를
> 다른 앱으로 취급하므로, `Scripts/bundle.sh`로 다시 빌드할 때마다 손쉬운 사용/입력 모니터링 목록에서
> 기존 항목을 (`-` 버튼으로) 지우고 새 번들을 다시 추가해야 합니다.
